...
class BasketPortfolioCreateServiceTest {
    ...
    @Test
    void create_overAllocation_throwsConflictBeforePersist() {
        UUID sourceId = UUID.randomUUID();
        PortfolioModelV1 source = PortfolioModelV1.builder()
                .id(sourceId)
                .owner("user-1")
        ...
        when(portfolioService.getPortfolioById(sourceId)).thenReturn(source);
        when(allocationAvailabilityService.getActiveAllocations(sourceId.toString()))
                .thenReturn(new HashMap<>(Map.of("INE040A01034", 8.0)));
        when(allocationAvailabilityService.getAvailableQuantity(any(), eq("INE040A01034"), eq(10.0), eq(0.0)))
                .thenReturn(2.0);
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> basketPortfolioCreateService.create(request))
                .isInstanceOf(org.springframework.web.server.ResponseStatusException.class)
                .satisfies(ex -> assertThat(((org.springframework.web.server.ResponseStatusException) ex)
                        .getStatusCode().value()).isEqualTo(409));

        verify(portfolioService, times(0)).createBasketPortfolio(any());
        verify(allocationLedgerService, times(0)).reserveAllocations(any(), any(), any());
    }
}
