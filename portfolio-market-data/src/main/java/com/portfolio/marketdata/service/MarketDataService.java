...
public class MarketDataService {
     ...
     */
    public Map<String, MarketData> convertHistoricalToMarketData(Map<String, HistoricalData> historicalData) {
        if (historicalData == null || historicalData.isEmpty()) {
            return Collections.emptyMap();
        }

        Map<String, MarketData> marketDataMap = new HashMap<>();
        historicalData.forEach((symbol, series) -> {
            String cleanedSymbol = cleanSymbol(symbol);
            marketDataMap.put(cleanedSymbol, MarketDataConverter.fromHistoricalData(series, cleanedSymbol));
        });
        ...
        List<java.util.concurrent.CompletableFuture<Map<String, MarketData>>> futures = chunks.stream()
            .map(chunk -> java.util.concurrent.CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    for (Map.Entry<String, HistoricalData> entry : response.getData().entrySet()) {
                        chunkResult.put(entry.getKey(),
                                MarketDataConverter.fromHistoricalData(entry.getValue(), entry.getKey()));
                    }
                    return chunkResult;
                } catch (Exception e) {
