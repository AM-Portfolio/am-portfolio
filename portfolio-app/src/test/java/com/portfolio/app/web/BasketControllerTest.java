...
class BasketControllerTest {
    ...
    @Test
    void listDrafts_DoesNotTreatDraftsAsBasketId() throws Exception {
        when(basketDraftService.listDrafts("user-123", null))
                .thenReturn(BasketDraftDtos.BasketDraftListResponse.builder()
                        .drafts(Collections.emptyList())
                        .draftCount(0)
                        .draftLimit(5)
                        .build());

        mockMvc.perform(get("/v1/basket/drafts").param("userId", "user-123"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.draftCount").value(0))
                .andExpect(jsonPath("$.draftLimit").value(5));

        verify(basketDraftService).listDrafts(eq("user-123"), eq(null));
    }
}
