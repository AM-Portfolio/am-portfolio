...
public class PortfolioAnalyticsFacade {
    ...
    public AdvancedAnalyticsResponse calculateAdvancedAnalytics(AdvancedAnalyticsRequest request) {
        ...
        if (request.isIncludeHeatmap()) {
            Heatmap heatmap = generateSectorHeatmap(request.getPortfolioId());
        ...
        if (request.isIncludeMovers()) {
            ...
            GainerLoser movers = getTopGainersLosers(request.getPortfolioId(), limit);
            responseBuilder.movers(movers);
        }
        
        // Include sector allocation if requested
        if (request.isIncludeSectorAllocation()) {
            SectorAllocation sectorAllocation = calculateSectorAllocations(request.getPortfolioId());
        ...
        if (request.isIncludeMarketCapAllocation()) {
            MarketCapAllocation marketCapAllocation = calculateMarketCapAllocations(request.getPortfolioId());
