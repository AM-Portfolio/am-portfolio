...
public class BasketSubstituteApplier {
    ...
    public BasketOpportunity applySubstitutesOnExisting(BasketOpportunity base, List<EquityHoldings> userHoldings,
        ...
        if (!subSymbols.isEmpty()) {
            try {
                Map<String, Double> fetched = marketDataService.getCurrentPrices(new ArrayList<>(subSymbols));
                prices = fetched != null ? fetched : Collections.emptyMap();
            } catch (Exception e) {
                log.warn("apply-substitutes price fetch fail-open: {}", e.getMessage());
        ...
        for (BasketItem originalItem : base.getComposition()) {
            ...
            for (SubstituteAssignment assignment : itemAssignments) {
                ...
                // Planning uses physical weight so reclaim/residual work when availableQuantity is 0.
                // Create-portfolio enforces ledger availability before persist (no over-alloc orphans).
                double subWeight = overlapCalculator.getPhysicalWeight(sub);
                ...
                BasketItem splitItem = BasketItem.builder()
                        ...
                        .userWeight(BasketUtils.round(overlapCalculator.getPhysicalWeight(sub)))
        ...
        // Keep peers with residual physical capacity after this apply so remaining
        // missing slots can still offer them (even when availableQuantity is 0).
        List<EquityHoldings> refreshHoldingsForAlts = new ArrayList<>();
        for (EquityHoldings h : userHoldings) {
            double phys = overlapCalculator.getPhysicalWeight(h);
            if (phys <= 0.01) {
                continue;
            }
            String isin = h.getIsin();
            double consumed = isin != null ? finalConsumed.getOrDefault(isin, 0.0) : 0.0;
            if ((phys - consumed) <= 0.01) {
                continue;
            }
            if (h.getAvailableQuantity() != null && h.getAvailableQuantity() <= 0
                    && h.getQuantity() != null && h.getQuantity() > 0) {
                // Tier builders filter availableQuantity > 0; expose residual owned qty for alts.
                refreshHoldingsForAlts.add(EquityHoldings.builder()
                        .isin(h.getIsin())
                        .symbol(h.getSymbol())
                        .sector(h.getSector())
                        .quantity(h.getQuantity())
                        .availableQuantity(h.getQuantity())
                        .weightInPortfolio(h.getWeightInPortfolio())
                        .averageBuyingPrice(h.getAverageBuyingPrice())
                        .currentPrice(h.getCurrentPrice())
                        .marketCapCategory(h.getMarketCapCategory())
                        .build());
            } else {
                refreshHoldingsForAlts.add(h);
            }
        }
        Map<String, List<EquityHoldings>> userSectorMap = refreshHoldingsForAlts.stream()
        ...
        overlapCalculator.refreshMissingAlternatives(newComposition, finalConsumed, refreshHoldingsForAlts,
    ...
    private void enrichBasketProfile(BasketOpportunity base) {
        if (base.getSectorialBasket() != null && base.getEtfConstituentIsins() != null) {
            // Client may forge sectorial=true with unknown dominant — force soft gate off.
            if (Boolean.TRUE.equals(base.getSectorialBasket())
                    && SectorNormalizer.isUnknown(base.getDominantSector())) {
                base.setSectorialBasket(false);
                base.setDominantSector(null);
            }
            return;
        }
    ...
    private void planReclaimForExplicitAssignments(
        ...
        for (SubstituteAssignment assignment : assignments) {
            ...
            double totalWeight = overlapCalculator.getPhysicalWeight(sub);
    ...
    private static boolean sectorsMatch(String missingSector, String substituteSector) {
        // Soft gate: Unknown/blank never hard-blocks apply (enrichment often incomplete).
        if (missingSector == null || missingSector.isBlank()
                || substituteSector == null || substituteSector.isBlank()
                || SectorNormalizer.isUnknown(missingSector)
                || SectorNormalizer.isUnknown(substituteSector)) {
            return true;
        }
