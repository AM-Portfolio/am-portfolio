    ...
    public BasketOpportunity calculateBasketQuantities(Double investmentAmount, BasketOpportunity opportunity,
        ...
        for (BasketItem item : opportunity.getComposition()) {
            ...
            if (item.getStatus() == ItemStatus.MISSING) {
                item.setBuyQuantity(0.0);
                item.setTargetQuantity((double) baseTargetQty);
                // Always size MISSING buys (includeHeld=true used to skip this and left buyQuantity=0).
                gapAmounts.put(item.getStockSymbol(), baseTargetAmount);
            }
        }

        if (!gapAmounts.isEmpty()) {
            List<String> priceWarnings = opportunity.getPriceWarnings() != null
                    ? new ArrayList<>(opportunity.getPriceWarnings())
                    : new ArrayList<>();
            for (BasketItem item : opportunity.getComposition()) {
                ...
                if (price == null || price <= 0) {
                    priceWarnings.add(item.getStockSymbol() + ":MISSING_PRICE");
                    item.setBuyQuantity(0.0);
                    continue;
                }
            ...
            if (!priceWarnings.isEmpty()) {
                opportunity.setPriceWarnings(priceWarnings);
            }
        }
