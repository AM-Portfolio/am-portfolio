...
public class BasketOverlapCalculator {
        ...
        if (prefetchedPrices != null) {
            ...
            for (String s : symbolsToFetch) {
                ...
                if (px == null || px <= 0) {
                    ...
                    resolvedPrices.put(s, BasketPriceResolver.ResolvedPrice.builder()
                            .price(px)
                            .quality(BasketPriceResolver.QUALITY_LIVE)
                            .asOf(null)
                            .build());
                }
            }
            if (!gaps.isEmpty()) {
                Map<String, BasketPriceResolver.ResolvedPrice> fetched =
                        basketPriceResolver.fetchResolvedPrices(gaps, allUserHoldings);
                resolvedPrices.putAll(fetched);
                for (Map.Entry<String, BasketPriceResolver.ResolvedPrice> e : fetched.entrySet()) {
                    if (e.getValue() != null && e.getValue().getPrice() != null) {
                        prices.put(e.getKey(), e.getValue().getPrice());
                    }
                }
            }
        } else {
            resolvedPrices = basketPriceResolver.fetchResolvedPrices(symbolsToFetch, allUserHoldings);
            prices = new HashMap<>();
            for (Map.Entry<String, BasketPriceResolver.ResolvedPrice> e : resolvedPrices.entrySet()) {
                if (e.getValue() != null && e.getValue().getPrice() != null) {
                    prices.put(e.getKey(), e.getValue().getPrice());
                }
            }
        ...
        if (etf.getHoldings() != null) {
            ...
            for (EtfHolding req : etf.getHoldings()) {
                BasketItem item = BasketItem.builder()
                        ...
                        .build();
                BasketPriceResolver.applyQuality(item, resolvedPrices.get(req.getSymbol()));
    ...
    public SectorProfile detectSectorProfile(EtfData etf) {
        ...
        double maxClassifiedWeight = 0;
        double totalWeight = 0;
        double classifiedWeight = 0;
        ...
        if (etf.getHoldings() != null) {
            for (EtfHolding h : etf.getHoldings()) {
                ...
                if (h.getSector() == null || SectorNormalizer.isUnknown(h.getSector())) {
                    continue; // Unknown must never drive sectorial exclusivity
                }
                String sector = SectorNormalizer.normalizeFine(h.getSector());
                if (SectorNormalizer.isUnknown(sector)) {
                    continue;
                }
                classifiedWeight += h.getWeight();
            ...
            for (Map.Entry<String, Double> entry : sectorWeights.entrySet()) {
                if (entry.getValue() > maxClassifiedWeight) {
                    maxClassifiedWeight = entry.getValue();
                    dominant = entry.getKey();
                }
            }
            // Require enough classified coverage and concentration within classified weight.
            if (totalWeight > 0
                    && classifiedWeight / totalWeight >= 0.80
                    && classifiedWeight > 0
                    && maxClassifiedWeight / classifiedWeight > 0.75) {
                isSectorial = true;
            } else {
                dominant = null;
            }
    ...
    /**
     * Physical portfolio weight for in-opportunity reallocation (apply-substitutes).
     * Unlike {@link #getAvailableWeight}, does not zero out when {@code availableQuantity}
     * is 0 — exclusivity is enforced by the caller via {@code consumedWeightByIsin}.
     */
    /**
     * Full holding weight for planning (apply/reclaim). Not inventory-gated —
     * create-portfolio enforces ledger availability before persist.
     */
    public double getPhysicalWeight(EquityHoldings h) {
        if (h == null || h.getWeightInPortfolio() == null) {
            return 0.0;
        }
        return h.getWeightInPortfolio();
    }
